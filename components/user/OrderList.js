...
const StatusSection = ({ title, orders, icon: Icon, color }) => {
  ...
  return (
    <Card className={`h-full ${color} border-l-4 ${
      color === 'bg-background' ? 'border-l-gray-300' :
      color.includes('yellow') ? 'border-l-yellow-500' :
      color.includes('blue') ? 'border-l-blue-500' :
      'border-l-green-500'
    }`}>
      <CardHeader className="py-4 px-4">
        <div className="flex items-center justify-between">
